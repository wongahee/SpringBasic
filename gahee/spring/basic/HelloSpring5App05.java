...
public class HelloSpring5App05 {
	public static void main(String[] args) {
	    ...
	    // 객체를 생성하는 코드를 작성했었음
	    // 하지만, 스프링을 사용하면 이러한 과정은 필요X

		// 스프링 프레임워크
	    // DI, IoC, AOP를 적용한 콘솔/GUI/웹 응용프로그램 개발에
	    // 사용하는 오픈소스 라이브러리
	    // 복잡한 절차를 요구하고 비용이 많이 드는 EJB에 비해
	    // 사용이 간단하고, 무료 서블릿 컨테이너에도
	    // 무난히 실행됨

		// 스프링을 이용한 개발의 필수요건은 인터페이스 기반 설계임
		...
		// 실행원리
	    // 예제에서 보듯 객체 생성 시 new 연산자를 사용하지 않고
	    ...
	    // BeanFactory에서 getBean이라는 메서드로
	    // 해당 객체를 가져와서 HelloSpring5Bean03 변수에 주입
	    // 이 변수의 sayHello 메서드를 호출하고 인사말 출력
		...
		bean = (HelloSpring5Bean03) bf.getBean("eng"); // 앞 ()는 형변환
